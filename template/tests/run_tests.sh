...
managed_distribs="jessie wheezy stretch buster centos6 centos7 archlinux"
...
start_container() {
  ...
  if grep -E "jessie|stretch|buster" <(echo $distrib_name) >/dev/null
  then
...
then
  ...
  [ "${distrib_name}" == "buster" ] && docker_image="multimediabs/plumb_unit:debian_buster" && init=systemd
